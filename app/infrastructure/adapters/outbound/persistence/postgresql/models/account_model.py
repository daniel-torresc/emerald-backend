"""
Account and AccountShare SQLAlchemy models.

This module defines:
- AccountModel: Financial account model (savings, credit card, loan, investment, etc.)
- AccountShareModel: Account sharing permissions (owner, editor, viewer)
...
class AccountModel(Base, TimestampMixin, SoftDeleteMixin, AuditFieldsMixin):
    """
    Financial account SQLAlchemy model.

    This is the ORM model for database persistence. Pure SQLAlchemy with no business logic.
    Business logic lives in domain.entities.account.Account.
    ...
    Relationships:
        owner: UserModel object who owns this account (via user_id)
        shares: List of AccountShareModel objects (who has access and permission level)
    ...
    Account Sharing:
        - Accounts can be shared with multiple users via AccountShareModel
        - Each share has a permission level (owner, editor, viewer)
    ...
    shares: Mapped[list["AccountShareModel"]] = relationship(
        "AccountShareModel",
        back_populates="account",
        foreign_keys="AccountShareModel.account_id",
        lazy="selectin",
    ...
    def __repr__(self) -> str:
        """String representation of AccountModel."""
        return (
            f"AccountModel(id={self.id}, name={self.account_name}, "
...
class AccountShareModel(Base, TimestampMixin, SoftDeleteMixin, AuditFieldsMixin):
    """
    Account sharing permissions SQLAlchemy model.

    This is the ORM model for database persistence. Pure SQLAlchemy with no business logic.
    Business logic lives in domain.entities.account_share.AccountShare.
    ...
    Relationships:
        account: AccountModel object being shared
        user: UserModel object who has access
    ...
    Example:
        ...
        partner_share = AccountShareModel(
            account_id=account.id,
            user_id=partner.id,
        ...
        advisor_share = AccountShareModel(
            account_id=account.id,
            user_id=advisor.id,
    ...
    def __repr__(self) -> str:
        """String representation of AccountShareModel."""
        return (
            f"AccountShareModel(id={self.id}, account_id={self.account_id}, "
