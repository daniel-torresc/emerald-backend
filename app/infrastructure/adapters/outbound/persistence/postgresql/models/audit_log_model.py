"""
AuditLog SQLAlchemy model for comprehensive audit trail.
...
class AuditLogModel(Base):
    """
    AuditLog SQLAlchemy model for tracking all system actions.

    This is the ORM model for database persistence. Pure SQLAlchemy with no business logic.
    Business logic lives in domain.entities.audit_log.AuditLog.
    ...
    user: Mapped[Optional["UserModel"]] = relationship(  # type: ignore
        "UserModel",
        lazy="selectin",
    )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"AuditLogModel(id={self.id}, user_id={self.user_id}, action={self.action}, "
